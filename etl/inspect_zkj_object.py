...
async def main():
    async with async_playwright() as p:
            ...
            return new Promise((resolve) => {
                ...
                ws.onmessage = (event) => {
                    ...
                    if (msg.id === 1 && msg.result) {
                        ...
                        const dims = hc.qDimensionInfo.map(d => ({ title: d.qFallbackTitle, expr: d.qGroupFieldDefs }));
                        const measures = hc.qMeasureInfo.map(m => ({ title: m.qFallbackTitle, expr: m.qDef }));
                        resolve({ dims, measures, grandTotals: hc.qGrandTotalRow?.map(g => g.qText) });
                        ws.close();
                    }
                };
        ...
        print("Dimensões e Métricas do Objeto Qlik ZKJqXsu:")
