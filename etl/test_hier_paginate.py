...
async def main():
    async with async_playwright() as p:
            ...
            ws.onmessage = async (event) => {
                ...
                if (msg.id === 1 && msg.result) {
                    ...
                    ws.send(JSON.stringify({
                        ...
                        "params": [{
                            ...
                            "qHyperCubeDef": {
                                ...
                                "qInitialDataFetch": [{ "qTop": 0, "qLeft": 0, "qHeight": pageSize, "qWidth": 12 }],
                    ...
                    if (currentTop < totalRows) {
                        ws.send(JSON.stringify({
                            "jsonrpc": "2.0", "id": 100 + currentTop,
                            "method": "GetHyperCubeData", "handle": hierObjHandle,
                            "params": ["/qHyperCubeDef", [{ "qTop": currentTop, "qLeft": 0, "qHeight": pageSize, "qWidth": qWidth }]]
                        }));
                    } else {
                        ws.close();
                        resolve({ totalRows, allRowsCount: allRows.length, sumV26: allRows.reduce((s, r) => s + (typeof r[3] === 'number' ? r[3] : 0), 0) });
                    }
                } else if (msg.id >= 100 && msg.result) {
                    const matrix = msg.result.qDataPages[0]?.qMatrix || [];
                    matrix.forEach(r => allRows.push(r.map(c => c.qNum !== 'NaN' && typeof c.qNum === 'number' ? c.qNum : c.qText)));
                    currentTop = allRows.length;
                    
                    if (currentTop < totalRows && matrix.length > 0) {
                        ws.send(JSON.stringify({
                            "jsonrpc": "2.0", "id": 100 + currentTop,
                            "method": "GetHyperCubeData", "handle": hierObjHandle,
                            "params": ["/qHyperCubeDef", [{ "qTop": currentTop, "qLeft": 0, "qHeight": pageSize, "qWidth": qWidth }]]
        ...
        print("Resultado da Paginação da Hierarquia:")
