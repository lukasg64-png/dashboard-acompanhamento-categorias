...
async def main():
    async with async_playwright() as p:
                ...
                ws.onmessage = (event) => {
                    ...
                    if (msg.id === 1 && msg.result) {
                        ...
                        ws.send(JSON.stringify({
                            ...
                            "handle": docHandle,
                            "params": [{
                                "qInfo": { "qType": "test_anomes_totals" },
                                "qHyperCubeDef": {
                                    "qDimensions": [
        ...
        print("Totais por Ano-Mes no Qlik Sense:")
        for r in result.get('rows', []):
            print(f"  Ano-Mes: {r[0]} | Receita Líquida: R$ {r[1]:,.2f} | Dias: {r[2]} até {r[3]}")
        await browser.close()
