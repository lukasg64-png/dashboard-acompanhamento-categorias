...
async def main():
    async with async_playwright() as p:
            ...
            return new Promise((resolve) => {
                ...
                ws.onopen = async () => {
                    try {
                        ...
                        // Hierarquia completa com Diretor, Distrital, Grupo, Subgrupo, Linha
                        const c = await send("CreateSessionObject", docHandle, [{
                            "qInfo": { "qType": "q_test_full_hier" },
                            "qHyperCubeDef": {
                                "qDimensions": [
                        ...
                        resolve({
                            totalRowsHier: totalRows,
                            sample: l.result.qLayout.qHyperCube.qDataPages[0]?.qMatrix?.slice(0, 5)
                        });
                        ws.close();
                    } catch (e) {
        ...
        print("RESULT FULL HIERARCHY:", json.dumps(res, indent=2, ensure_ascii=False))
        await browser.close()
