...
async def main():
    async with async_playwright() as p:
        ...
        # Testar obter lista de campos do App
        print("2. Consultando campos do App...")
                ...
                ws.onmessage = (event) => {
                    ...
                    if (msg.id === 1 && msg.result) {
                        ...
                        ws.send(JSON.stringify({
                            ...
                            "handle": docHandle,
                            "params": [{"qcx": 0, "qcy": 0}, {"qcx": 0, "qcy": 0}, 0, false, false]
                        }));
                    } else if (msg.id === 2 && msg.result) {
                        const tr = msg.result.qtr;
                        const tables = tr.map(t => ({
                            name: t.qName,
        ...
        print("Tabelas e Campos no Qlik:")
        for t in result.get('tables', []):
            print(f"\n📊 Tabela: {t['name']} ({t['rowCount']} linhas)")
            print(f"   Campos: {t['fields']}")
