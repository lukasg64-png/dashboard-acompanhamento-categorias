...
async def main():
    async with async_playwright() as p:
            ...
            return new Promise((resolve) => {
                ...
                ws.onmessage = (event) => {
                    ...
                    if (msg.id === 1 && msg.result) {
                        ...
                        // Query de Canais com as 3 medidas de período (Ago/26, Jul/26, Ago/25)
                        ws.send(JSON.stringify({
                            "jsonrpc": "2.0",
                            "id": 2,
                            ...
                            "handle": docHandle,
                            "params": [{
                                "qInfo": { "qType": "canais_3periodos" },
                                "qHyperCubeDef": {
                                    "qDimensions": [
                                        { "qDef": { "qFieldDefs": ["Canal"] } }
                                    ],
                                    "qMeasures": [
                                        { "qDef": { "qDef": "Sum({1<[Ano-Mes]={202608}>} [Receita Líquida])", "qLabel": "Ago_26" } },
                                        { "qDef": { "qDef": "Sum({1<[Ano-Mes]={202607}>} [Receita Líquida])", "qLabel": "Jul_26" } },
                                        { "qDef": { "qDef": "Sum({1<[Ano-Mes]={202508}>} [Receita Líquida])", "qLabel": "Ago_25" } }
                                    ],
        ...
        print("Canais nos 3 Períodos no Qlik Sense (Set Analysis {1}):")
        for r in result.get('rows', []):
            print(f"  {r[0]:<25} | Ago/26: R$ {r[1]:>12,.2f} | Jul/26: R$ {r[2]:>12,.2f} | Ago/25: R$ {r[3]:>12,.2f}")
        await browser.close()
