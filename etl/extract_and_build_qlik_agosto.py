"""
extract_and_build_qlik_agosto.py — Extrai dados diários completos de Agosto do Qlik Sense
com a fórmula exata de Resultado Líquido e gera os modelos de dados com vetores diários.
"""
...
async def fetch_all_qlik_agosto():
    os.makedirs(AGOSTO_DIR, exist_ok=True)
    t0 = time.time()
    print("\n" + "=" * 70)
    print("  EXTRAINDO MODELOS AGREGADOS DE AGOSTO DO QLIK SENSE ENTERPRISE")
    print("=" * 70)
    ...
    async with async_playwright() as p:
        ...
        print("  2/4 Executando extrações no Qlik Engine...")
        queries_js = '''async () => {
            ...
            return new Promise((resolve) => {
                ...
                ws.onmessage = async (event) => {
                    ...
                    if (msg.id === 1 && msg.result) {
                        ...
                        ws.send(JSON.stringify({
                            ...
                            "params": [{
                                ...
                                "qHyperCubeDef": {
                                    ...
                                    "qInitialDataFetch": [{ "qTop": 0, "qLeft": 0, "qHeight": 3000, "qWidth": 5 }],
                                    "qSuppressZero": true, "qSuppressMissing": true
                                }
                            }]
                        }));
                        ...
                        // Query 3: Hierarquia por Canal (Grupo + Subgrupo + Linha x Canal x Ano-Mes)
                        ws.send(JSON.stringify({
                            "jsonrpc": "2.0",
                            "id": 30,
                            "method": "CreateSessionObject",
                            ...
                            "params": [{
                                ...
                                "qHyperCubeDef": {
                                    ...
                                    "qInitialDataFetch": [{ "qTop": 0, "qLeft": 0, "qHeight": 8000, "qWidth": 6 }],
                                    "qSuppressZero": true, "qSuppressMissing": true
                                }
                            }]
                        }));
                ...
                setTimeout(() => resolve({ error: 'timeout', results }), 40000);
            });
        }'''
        ...
        print(f"  ✅ Consultas no Qlik Engine concluídas em {time.time() - t0:.2f}s!")
        return raw_res

def process_and_save_agosto_models(raw_res):
    print("\n  3/4 Compilando JSONs de Agosto com vetores de 31 dias...")
    t1 = time.time()
    ...
    print(f"  Registros -> Canais: {len(canais_raw)}, Categorias: {len(cats_raw)}, Hierarquia: {len(hier_raw)}")
    
    if not canais_raw:
        print("  ❌ Erro: nenhum dado retornado de Canais.")
        return
        
    cur_anomes = '2026-08'
    prev_mo_anomes = '2026-07'
    prev_yr_anomes = '2025-08'
    
    # 1. CANAIS SUMMARY COM VETORES DIÁRIOS
    df_c = pd.DataFrame(canais_raw, columns=['canal', 'anomes', 'dia', 'venda'])
    df_c['canal'] = df_c['canal'].apply(clean_str)
    df_c['anomes'] = df_c['anomes'].astype(str).str.strip()
    ...
    tot_mo = float(df_c[(df_c['anomes'] == prev_mo_anomes) & (df_c['dia'] <= d_max)]['venda'].sum())
    tot_yr = float(df_c[(df_c['anomes'] == prev_yr_anomes) & (df_c['dia'] <= d_max)]['venda'].sum())
    ...
    # 2. CATEGORIAS SUMMARY COM VETORES DIÁRIOS
    cats_list = []
    if cats_raw:
        ...
        df_cat['anomes'] = df_cat['anomes'].astype(str).str.strip()
    ...
    canais_hier_list = []
    if hier_raw:
        df_h = pd.DataFrame(hier_raw, columns=['grupo', 'subgrupo', 'linha', 'canal', 'anomes', 'venda'])
        ...
        df_h['anomes'] = df_h['anomes'].astype(str).str.strip()
        df_h['venda'] = pd.to_numeric(df_h['venda'], errors='coerce').fillna(0.0)
        
        for (g, sg, l, c), h_df in df_h.groupby(['grupo', 'subgrupo', 'linha', 'canal']):
            v_cur = float(h_df[h_df['anomes'] == cur_anomes]['venda'].sum())
            v_mo = float(h_df[h_df['anomes'] == prev_mo_anomes]['venda'].sum())
            v_yr = float(h_df[h_df['anomes'] == prev_yr_anomes]['venda'].sum())
    ...
    hier_detalhada_list = []
    if hier_raw:
        df_hd = pd.DataFrame(hier_raw, columns=['grupo', 'subgrupo', 'linha', 'canal', 'anomes', 'venda'])
        ...
        df_hd['anomes'] = df_hd['anomes'].astype(str).str.strip()
        df_hd['venda'] = pd.to_numeric(df_hd['venda'], errors='coerce').fillna(0.0)
        ...
        for (g, sg, l), h_df in df_hd.groupby(['grupo', 'subgrupo', 'linha']):
            ...
            v_dig_cur = float(h_df[(h_df['anomes'] == cur_anomes) & (h_df['canal_grupo'] == 'digital')]['venda'].sum())
            v_dig_mo = float(h_df[(h_df['anomes'] == prev_mo_anomes) & (h_df['canal_grupo'] == 'digital')]['venda'].sum())
            v_dig_yr = float(h_df[(h_df['anomes'] == prev_yr_anomes) & (h_df['canal_grupo'] == 'digital')]['venda'].sum())
            
            v_dt_cur = float(h_df[(h_df['anomes'] == cur_anomes) & (h_df['canal_grupo'].isin(['digital', 'tele']))]['venda'].sum())
            v_dt_mo = float(h_df[(h_df['anomes'] == prev_mo_anomes) & (h_df['canal_grupo'].isin(['digital', 'tele']))]['venda'].sum())
            v_dt_yr = float(h_df[(h_df['anomes'] == prev_yr_anomes) & (h_df['canal_grupo'].isin(['digital', 'tele']))]['venda'].sum())
    ...
    print(f"  4/4 JSONs salvos em {time.time() - t1:.2f}s!")
    print(f"  📊 Total Vendas Agosto D-1 (01 a {d_max}): R$ {tot_cur:,.2f} | YoY: {kpis['yoy_pct']:+.1f}% | MoM: {kpis['mom_pct']:+.1f}%")

def main():
    raw_res = asyncio.run(fetch_all_qlik_agosto())
    if raw_res and not raw_res.get('error'):
        process_and_save_agosto_models(raw_res)
    else:
        print("  ⚠️ Falha ao extrair do Qlik.")
