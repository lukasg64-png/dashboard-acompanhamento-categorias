...
for m in re.finditer(r'wire:snapshot="([^"]+)"', html):
    ...
    data = json.loads(raw)
    if data.get('memo', {}).get('name') == 'project.new.select':
        comp = data
        break

print("Chamando setType('private-deploy-key') em project.new.select...")
livewire_payload = {
    "_token": page_csrf,
    "components": [
        {
            "snapshot": json.dumps(comp),
            "updates": {},
            "calls": [
                {
                    "path": "",
...
try:
    ...
    comp_resp = res_data['components'][0]
    snapshot = json.loads(comp_resp['snapshot'])
    print("Current step:", snapshot['data'].get('current_step'))
    print("Type:", snapshot['data'].get('type'))
    print("Servers:", snapshot['data'].get('servers'))
    print("Destination UUID:", snapshot['data'].get('destination_uuid'))
    print("Effects:", comp_resp.get('effects'))
