            ...
            return new Promise((resolve) => {
                ...
                ws.onopen = async () => {
                    try {
                        ...
                        // 1. Test hypercube with Diretor only
                        const c1 = await send("CreateSessionObject", docHandle, [{
                            "qInfo": { "qType": "q_test_dir" },
                            "qHyperCubeDef": {
                                "qDimensions": [
                        ...
                        // 2. Test hypercube with Diretor + Desc_Grupo
                        const c2 = await send("CreateSessionObject", docHandle, [{
                            "qInfo": { "qType": "q_test_dir_grupo" },
                            "qHyperCubeDef": {
                                "qDimensions": [
                                    { "qDef": { "qFieldDefs": ["Diretor"] } },
                        ...
                        resolve({
                            diretores: l1.result.qLayout.qHyperCube.qDataPages[0]?.qMatrix,
                            diretor_grupos: l2.result.qLayout.qHyperCube.qDataPages[0]?.qMatrix?.slice(0, 10)
                        });
                        ws.close();
                    } catch (e) {
