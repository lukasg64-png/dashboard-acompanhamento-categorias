...
EXPR = "Sum([Valor Líquido] - [Valor Receita Recarga] + ([Valor Receita Recarga] * 4.63)/100)"
...
async def main():
    async with async_playwright() as p:
            ...
            return new Promise((resolve) => {
                ...
                ws.onmessage = (event) => {
                    ...
                    if (msg.id === 1 && msg.result) {
                        ...
                        // Consulta de canais com a fórmula exata de Resultado Líquido
                        ws.send(JSON.stringify({
                            "jsonrpc": "2.0",
                            "id": 2,
        ...
        print("Totais por Canal e Ano-Mes no Qlik Sense (Fórmula Exata):")
        for r in result.get('rows', []):
            print(f"  {r[0]:<25} | {r[1]} | R$ {r[2]:,.2f}")
        await browser.close()
